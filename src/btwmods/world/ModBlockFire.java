...
public class ModBlockFire extends FCBlockFire {
	...
	public boolean canBlockCatchFire(IBlockAccess blockAccess, int x, int y, int z) {
		return super.canBlockCatchFire(blockAccess, x, y, z) && blockCatchFireAllowed(blockAccess, x, y, z);
	}

	@Override
	protected void TryToDestroyBlockWithFire(World world, int x, int y, int z, int var5, Random rnd, int var7) {
		if (destroyBlockWithFireAllowed(world, x, y, z))
	...
	public static boolean blockCatchFireAllowed(IBlockAccess blockAccess, int x, int y, int z) {
		return false;
	}
}
